...
DIC <- bind_rows(
  predictors %>% mutate(tref = "2002",
                        Version_ID_group = "DIC_climatology",
                        MLR_basins = "none"),
...
DIC_temp <- DIC %>%
  filter(tref != 1800,
         Version_ID_group == "Standard case",
         MLR_basins == 3) %>%
  mutate(Version_ID_group = "dCant + temp")
...
DIC_DIC <- DIC %>%
  filter(tref %in% c("2004", "2014"),
         Version_ID_group %in% c("Standard case", "dCant + temp"),
         MLR_basins == 3) %>%
  mutate(Version_ID_group = if_else(
    Version_ID_group == "Standard case",
    "DIC",
    "DIC + temp"
  ))
...
for (i in 1:nrow(cases)) {
  ...
  acidification_trend %>%
    write_csv(paste0(path_OIA,"OIA_",i_Version_ID_group,"_",i_MLR_basins,"_DIC_addition.csv"))

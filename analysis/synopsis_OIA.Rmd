...
saturation_horizon_uncertainty_all_biome %>%
  ...
  mutate(value = paste(round(std_case,1), "±", round(sd,1))) %>% 
...
saturation_horizon_uncertainty_all_biome_change <-
  additive_uncertainty(df = saturation_horizon_uncertainty_all_biome_change %>% 
                         select(-pCO2))[[1]]
...
saturation_horizon_uncertainty_all_biome_change <-
  right_join(
    saturation_horizon_uncertainty_all_biome_change %>%
      select(-std_case),
    acidification_saturation_horizon_levels_change_biome %>%
      select(-c(delta_depth_sd, delta_grid_cells, period)) %>%
      pivot_longer(contains("depth"),
                   names_to = "variable",
                   values_to = "std_case") %>% 
...
full_join(
  saturation_horizon_uncertainty_all_biome_change_combined %>%
    pivot_longer(contains("depth"),
                 values_to = "delta_depth"),
  saturation_horizon_uncertainty_all_biome_combined %>%
    pivot_longer(contains("depth"),
                 values_to = "depth")
) %>% arrange(biome, sat_level, name, tref) %>%
  kable() %>%
  kable_styling() %>%
  scroll_box(height = "300px")

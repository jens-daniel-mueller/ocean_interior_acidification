...
inner_join(
  ...
  ggplot(aes(tref, depth,
             col = sat_level, fill = sat_level)) +
  geom_path() +
...
inner_join(acidification_saturation_horizon_levels_change,
           basinmask) %>% 
  group_by(basin, tref, sat_level) %>%
  summarise(delta_depth = weighted.mean(delta_depth, area)) %>%
  ungroup() %>% 
  mutate(sat_level = as.factor(sat_level)) %>% 
  ggplot(aes(tref, delta_depth, 
             col = sat_level, fill = sat_level)) +
  geom_path() +
  geom_point(shape = 21, col = "grey20") +
  scale_color_viridis_d() +

  ...
  left_join(
    buffer_trend_uncertainty %>%
      select(
        ...
        depth,
        tref,
        contains(OIA_variables)
      ) %>%
      select(-c(contains("delta"), "tco2")) %>%
      pivot_longer(
        any_of(OIA_variables),
        names_to = "variable",
        values_to = "added"
      ),
    acidification_trend_uncertainty %>%
...
buffer_trend_addition <-
  full_join(
    buffer_trend %>%
      select(lon, lat, depth, tref, contains(OIA_variables)) %>%
      select(-c(contains("delta"), "tco2")) %>%
      pivot_longer(any_of(OIA_variables),
                   names_to = "variable",
                   values_to = "added"),
    acidification_trend %>%
      select(lon, lat, depth, tref, contains(OIA_variables)) %>%
...
additive_uncertainty <- function(df) {
  ...
  df_factorial_eMLR_stat <-
    bind_rows(
      ...
      df_factorial_eMLR_stat %>%
        ...
        select(-tref) %>%
        group_by(across(-sd)) %>%
        summarise(sd = scaling_1994 * sqrt(sum(sd ^ 2, na.rm = TRUE))) %>%
        ungroup() %>%
        mutate(tref = 1994)
    )
  ...
  # integrate uncertainty components over time
...
acidification_depth_layer_uncertainty <- acidification_trend_uncertainty %>%
...
# calculate regional mean trends
...
acidification_depth_layer_total_trend_regional <-
  ...
  summarise(delta_total_sd = wsd(delta_total, area),
            delta_total = wmean(delta_total, area)) %>%
  ungroup()
...
acidification_depth_layer_total_trend_global <-
  ...
  group_by(depth, biome, tref, variable) %>%
  summarise(delta_total_sd = wsd(delta_total, area),
            delta_total = weighted.mean(delta_total, area)) %>%
  ungroup()
...
# calculate regional mean trends for uncertainty
...
acidification_depth_layer_uncertainty_trend_contribution <-
  acidification_depth_layer_uncertainty_trend[[2]]

acidification_depth_layer_uncertainty_trend <-
  acidification_depth_layer_uncertainty_trend[[1]]

# join trends and uncertainty

acidification_depth_layer_total_trend <-
  full_join(
    acidification_depth_layer_total_trend %>%
      select(-delta_total_sd),
    acidification_depth_layer_uncertainty_trend %>%
...
acidification_depth_layer_total_zonal <-
  ...
  summarise(
    delta_total_sd = sd(delta_total, na.rm = TRUE),
...
acidification_depth_layer_uncertainty_zonal <-
  additive_uncertainty(acidification_depth_layer_uncertainty_zonal)[[1]]
...
for (i_var in OIA_variables) {
  ...
  p_trends <-
    ...
    geom_point(aes(pCO2, delta_total, col = biome, group = biome, size = biome),
              shape = 21,
              fill = "white") +
...
buffer_addition_depth_layer_trend <-
  inner_join(buffer_addition_depth_layer,
             biomemask) %>%
  filter(variable %in% buffer_variables_addition) %>%
  group_by(depth, biome, tref, variable) %>%
  summarise(sensitivity_sd = wsd(sensitivity, area),
            sensitivity = wmean(sensitivity, area)) %>%
  ungroup()
...
rm(buffer_addition_depth_layer_trend_global)
...
buffer_addition_depth_layer_trend %>%
  filter(year %in% c(1800, 2014)) %>%
  group_by(depth, biome, variable) %>%
  mutate(sensitivity_change = 100 * abs(sensitivity) / abs(first(sensitivity)) - 100) %>%
  ungroup() %>%
  filter(year == 2014,
         biome == "Global")
...
buffer_addition_depth_layer_zonal <-
  ...
  filter(variable %in% buffer_variables_addition) %>%
  group_by(depth, tref, variable, lat) %>%
  summarise(sensitivity_sd = sd(sensitivity, na.rm = TRUE),
            sensitivity = mean(sensitivity, na.rm = TRUE)) %>%
  ungroup() %>% 
  mutate(tref = as.factor(tref))

buffer_addition_depth_layer_zonal %>% 
  ggplot(aes(sensitivity, lat, col = tref)) +
...
buffer_addition_depth_layer_zonal %>% 
  ...
  geom_vline(xintercept = 0) +
  geom_path() +
  scale_color_viridis_d(labels = c("1","","2","")) +
  facet_grid(depth~variable, scales = "free_x")
...
for (i_var in buffer_variables_addition) {
  ...
  buffer_addition_depth_layer_trend %>%
    ...
    scale_x_continuous(breaks = seq(280, 400, 40)) +
    scale_color_manual(values = biome_colors) +
    scale_fill_manual(values = biome_colors_global) +
    scale_discrete_manual("linewidth", values = c(1, rep(0.5, 5))) +
    scale_size_manual(values = c(1.5, rep(0, 5))) +
    guides(
      fill = guide_legend(ncol = 2),
  ...
  if (i_var %in% c(OIA_variables_core, "pH")) {
    folder <- "supplement"
  } else {
    folder <- "all_variables"
  }
...
buffer_addition_depth_layer_trend %>%
  ...
  summarize(delta_sensitvity = 100 * (last(sensitivity) - first(sensitivity)) /
              first(sensitivity)) %>%
  ungroup()
...
for (i_var in OIA_variables) {
  ...
  for (i_depth in integration_depths) {
    # i_depth <- 500
    ...
    acidification_volume_binned <-
      ...
      group_by(tref, delta_total, variable, depth_integration) %>%
...
acidification_volume_binned_all <-
  ...
  mutate(
    ...
    volume_rel = 100 * volume / volume_total
  )


acidification_volume_binned_all %>%
  ggplot() +
  geom_step(aes(delta_total,
                volume_rel,
                col = tref,
                group = tref),
            direction = "mid") +
...
y_max_vol <- acidification_volume_binned_all %>%
  filter(
    variable %in% c("OmegaAragonite",
                    "H_free",
                    "tco2"),
    depth_integration == depth_integration_acidification
  ) %>%
  summarise(max(volume_rel)) %>%
  pull() * 1.05
...
acidification_volume_binned_all %>%
  mutate(
    delta_total_num = case_when(
      variable == "OmegaAragonite" & delta_total == "(-Inf,-0.8]" ~ -0.9,
      variable == "OmegaAragonite" & delta_total == "(-0.8,-0.7]" ~ -0.8,
      variable == "OmegaAragonite" & delta_total == "(-0.7,-0.6]" ~ -0.7,
      variable == "OmegaAragonite" & delta_total == "(-0.6,-0.5]" ~ -0.6,
      variable == "OmegaAragonite" & delta_total == "(-0.5,-0.4]" ~ -0.5,
      variable == "OmegaAragonite" & delta_total == "(-0.4,-0.3]" ~ -0.4,
      variable == "OmegaAragonite" & delta_total == "(-0.3,-0.2]" ~ -0.3,
      variable == "OmegaAragonite" & delta_total == "(-0.2,-0.1]" ~ -0.2,
      variable == "OmegaAragonite" & delta_total == "(-0.1,0]"    ~ -0.1,
      variable == "OmegaAragonite" & delta_total == "(0, Inf]"    ~ -0.0,
      variable == "H_free" & delta_total ==  "(-Inf,0]"   ~ -0.4,
      variable == "H_free" & delta_total ==  "(0,0.4]"    ~ 0,
      variable == "H_free" & delta_total ==  "(0.4,0.8]"  ~ 0.4,
      variable == "H_free" & delta_total ==  "(0.8,1.2]"  ~ 0.8,
      variable == "H_free" & delta_total ==  "(1.2,1.6]"  ~ 1.2,
      variable == "H_free" & delta_total ==  "(1.6,2]"    ~ 1.6,
      variable == "H_free" & delta_total ==  "(2,2.4]"    ~ 2,
      variable == "H_free" & delta_total ==  "(2.4,2.8]"  ~ 2.4,
      variable == "H_free" & delta_total ==  "(2.8,3.2]"  ~ 2.8,
      variable == "H_free" & delta_total ==  "(3.2, Inf]" ~ 3.2,
      variable == "tco2" & delta_total == "(-Inf,0]" ~ -10,
      variable == "tco2" & delta_total == "(0,10]"   ~ 0,
      variable == "tco2" & delta_total == "(10,20]"  ~ 10,
      variable == "tco2" & delta_total == "(20,30]"  ~ 20,
      variable == "tco2" & delta_total == "(30,40]"  ~ 30,
      variable == "tco2" & delta_total == "(40,50]"  ~ 40,
      variable == "tco2" & delta_total == "(50,60]"  ~ 50,
      variable == "tco2" & delta_total == "(60,70]"  ~ 60,
      variable == "tco2" & delta_total == "(70,80]"  ~ 70,
      variable == "tco2" & delta_total == "(80, Inf]"~ 80
    ),
    delta_total_max = case_when(
      variable == "OmegaAragonite" ~ -0.5,
      variable == "H_free" ~ 2,
      variable == "tco2" ~ 50,
    )
  ) %>% 
...
acidification_volume_binned_all_peak_sum <-
  ...
  group_by(variable, tref, depth_integration, delta_total_max) %>%
  summarise(volume_rel = sum(volume_rel)) %>%
  ungroup()
...
acidification_volume_binned_all_peak_sum <-
  ...
  mutate(
    ...
    x_pos = delta_total_max * 1.2,
    y_pos = y_max_vol * 0.8 + (tref - 2004) / 2,
    label_text = paste(round(volume_rel), "%")
  )

acidification_volume_binned_all_peak_sum %>%
  mutate(depth_integration = as.factor(depth_integration)) %>%
  ggplot(aes(tref, volume_rel, col = depth_integration)) +
  geom_point() +
  facet_wrap( ~ variable)
...
p_vol_OmegaAragonite <-
  ...
  # geom_rect(aes(xmax = delta_total_num+0.1,
  #               xmin = delta_total_num,
  #               ymax = volume_rel,
  #               ymin = volume_rel,
  #               col = tref),
...
for (i_var in OIA_variables) {
  ...
  i_contours <- i_contours[-length(i_contours)]
  i_contours <- i_contours[!i_contours == 0]
  
  for (i_i_contours in i_contours) {
    # i_i_contours <- i_contours[2]
    
    acidification_bottom_binned <-
      i_acidification_bottom_raw %>%
      filter(abs(delta_total) >= abs(i_i_contours)) %>% 
      group_by(tref, variable) %>%
      summarise(area = sum(area, na.rm = TRUE)) %>%
      ungroup() %>% 
      mutate(delta_total = i_i_contours)
    
    if (exists("acidification_bottom_binned_all")) {
      acidification_bottom_binned_all <-
        bind_rows(acidification_bottom_binned_all,
                  acidification_bottom_binned)
    }
    
    if (!exists("acidification_bottom_binned_all")) {
      acidification_bottom_binned_all <-  acidification_bottom_binned
    }
...
rm(acidification_bottom_binned)

# rm(acidification_bottom_binned_all)


acidification_bottom_binned_all <-
  acidification_bottom_binned_all  %>%
  mutate(
    # delta_total = as.factor(delta_total),
    tref = as.factor(tref),
    # delta_total = fct_relevel(delta_total, "(0, Inf]", after = Inf),
    area_rel = 100 * area / bottom_area_total
  )
...
acidification_bottom_binned_all %>%
  ...
  labs(y = "Acidified bottom (% of total top 3000m bottom area)") +
  theme(
    strip.text.x = element_markdown(),
    legend.title = element_blank(),
    axis.text.x = element_text(angle = 90, hjust = 1),
...
acidification_bottom_binned_all %>%
  filter(variable %in% OIA_variables_core) %>%
  ggplot() +
  geom_path(aes(delta_total,
                area_rel,
                group = delta_total)) +
  geom_point(aes(delta_total,
                 area_rel,
                 fill = tref),
             shape = 21) +
  scale_color_manual(values = three_year_colors) +
  scale_fill_manual(values = three_year_colors) +
  labs(y = "Acidified bottom\n(% of total top 3000m bottom area)",
...
ggsave(
  here::here("output/main/acidification_bottom_binned.png"),
  width = 6,
  height = 3,
  dpi = 600,
  bg = "white"
...
acidification_volume <-
  full_join(undersaturated_volume_all,
            ocean_volume_integration_depth) %>% 
...
acidification_volume <-
  acidification_volume %>%
  arrange(tref) %>%
  group_by(mineral, depth_integration, sat_level) %>%
  mutate(
    volume_loss =
...
acidification_volume <-
  acidification_volume %>%
  select(-c(volume_total, volume_undersat)) %>% 
  ...
  acidification_volume %>%
    ...
    map(
      ~ ggplot(data = .x,
        ...
        facet_grid(mineral ~ depth_integration)
    )
)
...
acidification_volume_print %>%
  ...
  facet_grid(estimate ~ depth_integration, scales = "free_y")




acidification_volume_print %>%
  filter(depth_integration == depth_integration_saturation,
         sat_level == 1)
...
acidification_volume_print %>%
  ...
  geom_point(aes(fill = sat_level,
                 shape = sat_level)) +
  geom_text(data = . %>% 
              filter(estimate == "Loss of sat. volume\n(% preind. vol.)",
                     sat_level == 4,
                     tref != 1800),
            aes(label = tref),
            nudge_y = -7,
...
ggsave(
  here::here(
    paste0(
      "output/supplement/global_sections_all_variables_absolute.png"
    )
  ),
  width = 6,
  height = 8,
...
```{r compute_mean_profiles_total_acidification_biomes_uncertainty, eval=FALSE}

acidification_trend_uncertainty_coverage <-
  acidification_trend_uncertainty %>%
  filter(depth == 0) %>%
...
buffer_profiles_biome_regional <- buffer_profiles_biome %>%
  group_by(tref, biome, depth, variable) %>%
  summarise(
    sensitivity_sd = wsd(sensitivity, area),
    sensitivity = wmean(sensitivity, area)
  ) %>%
  ungroup()
...
ggsave(
  here::here(
    paste0(
      "output/main/profiles_OmegaAragonite_biomes.png"
    )
  ),
  width = 7,
  height = 6,
...
acidification_profiles_biome_long <-
  acidification_profiles_biomes %>%
  filter(str_detect(variable, "_delta_total")) %>% 
  mutate(variable = str_remove(variable, "_delta_total")) %>%
  filter(depth <= ocean_depth,
         variable %in% OIA_variables)

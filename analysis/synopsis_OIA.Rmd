...
cwc <-
  ...
  filter(SPECIES %in% c("Lophelia pertusa",
                        ...
                        "Solenosmilia variabilis"))
  # group_by(lon, lat) %>%
  # summarise(DEPTH_MIN = min(DEPTH_MIN),
...
ggsave(
  here::here(
    paste0(
      "output/supplement/maps_depth_layer_pteropod_dissolution.png"
    )
  ),
  width = 10,
  height = 5,
...
cwc_saturation_state <-
  inner_join(cwc %>% mutate(obs_id = row_number()),
             acidification_trend,
             relationship = "many-to-many")

ggplot() +
  geom_raster(data = acidification_trend %>% distinct(lon, lat),
              aes(lon, lat),
              fill = "grey70") +
  geom_raster(data = cwc %>% distinct(lon, lat),
              aes(lon, lat),
              fill = "red") +
  coord_quickmap()

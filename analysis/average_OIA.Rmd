...
```{r read_buffer_changes_DIC_addition, eval=FALSE}
...
# Select buffer variables

```{r buffer_variables, eval=FALSE}
...
```{r depth_layer_buffer_addtion_climatology, eval=FALSE}
...
for (i_var in OIA_variables[-6]) {
  ...
  contours <- labels_breaks(i_var)$i_breaks
  ...
  for (i_contour in contours) {
    ...
    acidification_volume_global <- left_join(acidification_horizon,
                                             basinmask) %>%
    ...
    acidification_volume_regional <- left_join(acidification_horizon,
                                             basinmask) %>%
      mutate(volume = depth * area) %>%
      group_by(tref, variable, contour, basin) %>%
      summarise(volume = sum(volume)) %>%
...
full_join(ocean_volume,
  ...
  scale_x_continuous(breaks = c(1994, 2004, 2014),
                     limits = c(1994, 2019)) +
  labs(y = "Acidified volume of top 3000m global ocean (%)") +
  theme(
...
ggsave(
  here::here(
    "output/publication/acidification_volumes_region.png"
  ),
  width = 10,
  height = 8,
  dpi = 600,

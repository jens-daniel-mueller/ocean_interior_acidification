...
ALOHA %>%
  filter(between(depth,990,1510)) %>% 
  ggplot(aes(value, depth, col = depth_int, fill = year)) +
...
ALOHA %>%
  ...
  geom_point(data = . %>% filter(!is.na(depth_int)),
             aes(col = date)) +
  scale_color_viridis_c(trans = "date") +
  scale_y_continuous(trans = trans_reverser("sqrt"),
                     breaks = depth_labels) +
...
bats_bottle %>% 
  filter(depth_int == 600, name == "tco2") %>% 
  ggplot(aes(date, depth)) +
  geom_point()
...
pdf(
  here::here("output/all_variables/BATS_HOTS_time_series.pdf"),
  width = 14,
  height = 8
)
...
pdf(
  here::here("output/all_variables/BATS_HOTS_time_series_detrended.pdf"),
  width = 14,
  height = 8
)
...
pdf(
  here::here("output/all_variables/BATS_HOTS_detrended_monthly_climatology.pdf"),
  width = 14,
  height = 8
)
...
detrended %>%
  ...
  map(
    ~ ggplot(data = .x) +
      # geom_path(aes(month, value_detrended, group = year), col="grey80") +
...
pdf(
  here::here(
    "output/all_variables/BATS_HOTS_time_series_deasonalized.pdf"
  ),
  width = 14,
  height = 8
)
...
pdf(
  here::here(
    "output/all_variables/BATS_HOTS_time_series_deasonalized_annual.pdf"
  ),
  width = 14,
  height = 8
)
...
TS_trends %>%
  ...
  labs(y = "Depth (m)",
       x = "Long term trend") +
  # scale_y_continuous(trans = trans_reverser("sqrt")) +
  coord_cartesian(expand = 0, ylim = c(3000, 0)) +
  facet_wrap( ~ name, scales = "free_x",
              ncol = 3) +
  theme(legend.position = c(0.6, 0.1))

ggsave(
  here::here("output/all_variables/BATS_HOTS_annual_mean_trend_profile.pdf"),
  width = 8,
  height = 14
)

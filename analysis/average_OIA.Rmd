...
OIA_variables <- OIA_variables[-c(3,4,6,7,10)]
OIA_variables_core <- OIA_variables[c(3,5)]
...
ggsave(
  here::here("output/supplement/ALOHA_tco2_all_data_profiles.pdf"),
  width = 8,
  height = 8
)
...
ggsave(
  here::here("output/supplement/BATS_tco2_all_data_profiles.pdf"),
  width = 8,
  height = 8
)
...
acidification_depth_layer_total_relative <-
  ...
  filter(tref %in% c(1994, 2014),
         variable %in% OIA_variables[c(1,3,5)],
...
for (i_var in OIA_variables[c(1,2,3,5)]) {
...
acidification_profiles %>% 
  ...
  summarise(across(
    .cols = gamma:H_free_delta_interval,
    .fns = ~ wmean(.x, area),
    .names = "{.col}-mean"
  ),
  across(
...
buffer_profiles_biome <-
  inner_join(biomemask,
             buffer_trend_addition %>% select(-basin_AIP))

...
ggsave(
  here::here("output/all_variables/BATS_ALOHA_annual_mean_trend_profile.pdf"),
  width = 8,
  height = 14
)
...
acidification_profiles_station <-
  acidification_profiles_station %>%
    ...
    mutate(across(is.numeric, ~ .x - first(.x))) %>% 
    ungroup() %>% 
    filter(tref == 20) %>% 
...
x_axis_labels_stations <-
  c(
    ...
    "talk" = "ΔAT<br>(µmol kg<sup>-1</sup>)"
  )

bind_rows(
  TS_trends %>%
    filter(term == "year") %>%
    select(station, variable = name, depth = depth_int, estimate) %>%
    mutate(
      driver = "observed",
      tref = "observed\nperiod",
...
ggsave(
  here::here(
    paste0(
      "output/publication/profiles_20yr_changes_station.png"
    )
  ),
  width = 14,
  height = 8,

...
acidification_depth_layer_total_zonal %>% 
  ggplot() +
  geom_ribbon(#data = . %>% filter(tref == 2014),
              aes(xmin = delta_total - delta_total_sd,
...
ggsave(
  here::here(
    paste0(
      "output/supplement/maps_depth_layer_all_var_total_changes_relative.png"
    )
  ),
  width = 10,
  height = 5,
...
for (i_var in OIA_variables) {
  ...
  ggsave(
    here::here(
      paste0("output/all_variables/maps_depth_layer_",i_var,"_total_changes_relative.png")
    ),
    width = 10,
    height = 4.5,
    dpi = 600,
...
for (i_var in OIA_variables[c(1,3,5)]) {
  # i_var <- OIA_variables[1]
...
ggsave(
  here::here(
    paste0(
      "output/all_variables/maps_depth_layer_all_var_total_changes_buffer_additional_relative.png"
    )
  ),
  width = 14,
  height = 4.5,
...
biome_co2_speciation %>%
  ...
  labs(x = "Absolute value",
       y = expression(Change ~ per ~ Delta * DIC ~ (µmol ^ {
         -1
       } ~ kg))) +
...
ggsave(
  here::here(
    paste0(
      "output/main/maps_saturation_horizon_absolute_change_state_aragonite_all_sat_level_2014.png"
    )
  ),
  width = 8,
  height = 7,
...
acidification_profiles_station_obs_reconstructed %>%
  ...
  # scale_color_brewer(palette = "Dark2",
  #                    labels = labels_tco2) +
  scale_fill_highcontrast(labels = labels_tco2,
                          reverse = TRUE) +
  scale_color_highcontrast(labels = labels_tco2,
                          reverse = TRUE) +
...
ggsave(
  here::here(
    paste0(
      "output/main/BATS_ALOHA_20_yr_changes_incl_biogeochemistry.png"
    )
  ),
  width = 8,
  height = 7,

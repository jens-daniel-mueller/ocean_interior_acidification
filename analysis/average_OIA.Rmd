...
depth_limits <- c(0,10,30,60,85,125,175,300,450,550,650,900,1500,2500,3500,5000)
depth_labels <- c(0,25,40,75,100,150,200,400,500,600,750,1000,2000,3000,4500)
...
TS_product_consistent %>%
  ggplot() +
  geom_point(aes(month, value, col = date), alpha = 0.5) +
  scale_color_viridis_c(trans = "date") +
...
TS_product_consistent %>%
  ggplot() +
  geom_point(aes(month, value), alpha = 0.2) +
  geom_path(data = TS_product_season,
            aes(month, mean, col = "Monthly mean")) +
  geom_point(
    data = TS_product_season,
  ...
  geom_path(data = TS_product_season,
            aes(month, median, col = "Monthly median")) +
  facet_wrap(depth_int ~ name, scales = "free_y")



TS_product_deseason <-
  full_join(TS_product_consistent,
            TS_product_season)
  
TS_product_deseason <-
TS_product_deseason %>% 
  mutate(value_deseason = value - median)
...
TS_product_deseason %>% 
  mutate(depth_distance = depth - as.numeric(as.character(depth_int))) %>% 
  ggplot(aes(date, value_deseason, col = depth_distance)) +
  geom_point() +
  scale_color_berlin() +
...
TS_product_deseason %>%
  ggplot(aes(value_deseason, depth, col = date)) +
  geom_point() +
  scale_color_viridis_c(trans = "date") +
  scale_y_continuous(trans = trans_reverser("sqrt"),
                     breaks = depth_limits) +
...
library(broom)
regressions <- TS_product_deseason %>%

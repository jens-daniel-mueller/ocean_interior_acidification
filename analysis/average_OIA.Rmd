...
col_names <-
  names(read_csv(
    paste0(path_ungrd_3d_ocn, "/spots/TS_product.csv"),
    n_max = 0
  ))
...
TS_product %>%
  filter(between(depth,4450,4550)) %>% 
...
depth_limits_shallow <- c(0,10, 20,30, 40,50, 70,80, 90,110, 145,155, 195,205, 340,360, 480,520)
depth_labels_shallow <- c(5,NA, 25,NA, 45,NA, 75,NA, 100,NA, 150,NA,  200,NA,  350,NA,  500, NA) 
...
TS_product %>% 
  arrange(depth) %>% 
  mutate(n_row = row_number()) %>% 
  ggplot(aes(n_row,depth,col=depth_int)) +
  geom_point() + 
  theme_bw()


TS_product %>%
  ggplot(aes(value, depth, col = depth_int)) +
  geom_point(alpha = 0.4) +
  scale_y_continuous(trans = trans_reverser("sqrt")) +
  facet_wrap( ~ name, scales = "free_x") +
  theme_bw() +
...
TS_product %>%
  ggplot(aes(value, depth, col = date)) +
  geom_point() +
...
TS_product %>% 
  ggplot(aes(date, value, fill=depth_int-depth)) +
  geom_point(shape = 21) +
  scale_fill_continuous_divergingx() +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(depth_int ~ name,
             scales = "free") +
  theme_bw()

library(broom)
detrended <- TS_product %>%
  nest(data = -c(depth_int, name)) %>%
  mutate(
...
detrended <-
  detrended %>%
  unnest(c(data, augmented), names_repair = "universal") %>% 
  select(name, depth_int, date = date...5, year, month, value = value...9, value_detrended = .resid)

detrended %>% 
  ggplot(aes(date, value_detrended)) +
  geom_point(alpha=0.4) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(depth_int ~ name,
             scales = "free") +
...
detrended %>%
  ggplot() +
  geom_point(aes(month, value_detrended, col = date), alpha = 0.5) +
  scale_color_viridis_c(trans = "date") +
  facet_wrap(depth_int ~ name, scales = "free_y")
...
detrended %>%
  ...
  geom_point(aes(month, mean, fill = "Monthly mean"),
             shape = 21,
             size = 2) +
  geom_point(aes(month, median, fill = "Monthly median"),
             shape = 21,
             size = 2) +
  facet_wrap(depth_int ~ name, scales = "free_y")
...
detrended %>% 
  ggplot(aes(date, value_deseason)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(depth_int ~ name,
             scales = "free") +
...
detrended_annual <-
  detrended %>%
  group_by(year, depth_int, name) %>%
  summarise(annual_mean_deseason = mean(value_deseason)) %>%
  ungroup() %>% 
  mutate(depth_int = as.numeric(as.character(depth_int)))


detrended_annual %>%
  ggplot(aes(annual_mean_deseason, depth_int, col = year, group=year)) +
  geom_path() +
  geom_point() +
  scale_color_viridis_c() +
  scale_y_continuous(trans = trans_reverser("sqrt"),
...
detrended_annual %>%
  filter(name == "tco2") %>% 
  ggplot(aes(year, depth_int, z = annual_mean_deseason)) +
  geom_contour_filled() +
...
regressions %>%
  ...
  geom_ribbon(aes(xmin = estimate - std.error,
...
ocean_volume_biome <- inner_join(ocean_bathymetry,
                           biomemask)
...
pdf(
  here::here("output/all_variables/maps_acidification_horizons.pdf"),
  width = 5,
  height = 7
)
...
for (i_var in OIA_variables[-6]) {
  ...
  for (i_contour in i_contours) {
    ...
    acidification_horizon <-
      ...
      group_by(lon, lat, tref, variable) %>%
    ...
    if (exists("acidification_horizon_all")) {
      acidification_horizon_all <-
        bind_rows(acidification_horizon_all,
                  acidification_horizon)
    }
    
    if (!exists("acidification_horizon_all")) {
      acidification_horizon_all <- acidification_horizon
    }

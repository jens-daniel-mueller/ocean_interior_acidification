...
four_year_colors <- c("#7a8483", "#55609e", "#ffc125", "#b43032")
...
for (i_var in OIA_variables) {
  ...
  p_trends <-
    ...
    geom_ribbon(aes(x = pCO2,
                ...
                alpha = 0.1) +
    geom_path(aes(pCO2, delta_total, col = biome, group = biome)) +
    geom_point(aes(pCO2, delta_total, col = biome, group = biome),
               size = 0.5) +
...
for (i_var in OIA_variables) {
  ...
  i_legend_title <-
    paste0(i_legend_title, " / <br>", str_remove(i_legend_title, "Δ"), " (2004)")
...
acidification_depth_layer_total_relative %>%
  ...
  p_map_mdim_robinson(
    ...
    dim_col = "variable",
    var = "delta_total_relative",
    legend_title = "Δ<sub>1800-2014</sub> / <br>Absolute<sub>2004</sub>",
    breaks = c(-Inf,seq(0,30,5),Inf),
    legend_position = "right"
  )
...
for (i_var in buffer_variables_addition) {
  ...
  p_maps <- i_buffer_addition_depth_layer %>%
    filter(tref == 2014) %>% 
    mutate(tref = paste0("1800-", tref)) %>% 
    p_map_mdim_robinson(
      dim_row = "depth",
  ...
  p_trends <-
    ...
    geom_ribbon(aes(x = pCO2,
                    ...
                    group = biome),
                alpha = 0.1) +
    geom_path(aes(pCO2, sensitivity, col = biome, group = biome)) +
    geom_point(aes(pCO2, sensitivity, col = biome, group = biome),
               size = 0.5) +
    scale_x_continuous(
      breaks = round(unique(acidification_depth_layer_total_trend$pCO2))) +
    scale_color_manual(values = biome_colors) +
...
biome_co2_speciation %>%
  ...
  pivot_wider(names_from = estimate,
              values_from = value) %>%
  ggplot(aes(absolute, sensitivity, 
             col = depth, 
             shape = biome,
             alpha = tref,
...
p_sat_change <- acidification_saturation_horizon_levels_change %>%
  p_map_mdim_robinson(
    dim_row = "sat_level",
    # dim_col = "mineral",
    col = "div",
...
acidification_volume_print %>%
  ...
  ggplot(aes(pCO2, value)) +
  labs(x = expression(Atmospheric ~ CO[2] ~ (ppm))) +
  scale_x_continuous(breaks = seq(280, 400, 20),
                     sec.axis = sec_axis(
                       ~ (. - 282.5) * 1.537,
                       breaks = seq(0,175,50),

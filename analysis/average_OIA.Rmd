...
```{r climatology_2002_Lauvset_buffer}
...
climatology_buffer_depth_layer <- m_depth_layer(climatology_buffer)
...
pdf(
  here::here("output/all_variables/maps_depth_layer_dCant_total_changes.pdf"),
  width = 14,
  height = 5
)
...
i_acidification_depth_layer_total %>%
  p_map_mdim_robinson(
    ...
    dim_row = "depth",
    var = "delta_total",
    legend_title = expression(Delta*C[ant]~(µmol~kg^{-1})),
    breaks = breaks
  )

ggsave(
  here::here(
    "output/publication/maps_depth_layer_dCant_total_changes.png"
  ),
  width = 12,
  height = 6,
  dpi = 600
...
breaks <- c(-Inf, seq(0, 0.003, 0.0003), Inf)
...
i_acidification_depth_layer_total %>%
  p_map_mdim_robinson(
    ...
    dim_row = "depth",
    var = "delta_total",
    legend_title = expression(Delta*H[free]^"+"~(µmol~kg^{-1})),
    breaks = breaks
  )

ggsave(
  here::here(
    "output/publication/maps_depth_layer_total_changes_hydrogen_ion.png"
  ),
  width = 12,
  height = 6,
  dpi = 600
...
```{r depth_layer_climatology_buffer}

# subset total change sections

climatology_buffer_depth_layer <-
  climatology_buffer_depth_layer %>%
  select(-contains("_delta"))
...
buffer_variables <- climatology_buffer_depth_layer %>% 
  distinct(variable) %>% 
  pull
...
pdf(
  here::here("output/all_variables/maps_depth_layer_climatology_buffer.pdf"),
  width = 14,
  height = 4
)
...
for (i_var in buffer_variables) {
  ...
  i_climatology_buffer_depth_layer <-
    climatology_buffer_depth_layer %>%
    filter(variable == i_var) %>%
    select(lon, lat, depth, tref, value)
  ...
  low <- i_climatology_buffer_depth_layer %>%
    filter(!is.na(value)) %>%
    pull(value) %>%
    quantile(c(quantile_low), names = FALSE)

  high <- i_climatology_buffer_depth_layer %>%
    filter(!is.na(value)) %>%
    pull(value) %>%
    quantile(c(quantile_high), names = FALSE)
...
rm(climatology_buffer_depth_layer,
   i_climatology_buffer_depth_layer,
   p_climatology,
   low, high)

...
# p_map_biome / p_map_basin +
#   plot_annotation(tag_levels = 'a')
# 
# ggsave(
#   here::here(
#     paste0(
#       "output/main/map_biomebasinmask.png"
#     )
#   ),
#   width = 8,
#   height = 6,
...
two_year_colors <- c("#d62828", "#003049")
...
```{r depth_layer_total_change_multilayer_maps, eval=FALSE}
...
ggsave(
  here::here(
    "output/main/maps_depth_layer_total_changes_hydrogen_ion_isolines.png"
  ),
  width = 10,
  height = 6,
  dpi = 600,
...
ggsave(
  here::here(
    "output/main/maps_depth_layer_total_changes_hydrogen_ion_filled_isolines.png"
  ),
  width = 10,
  height = 6,
  dpi = 600,
...
buffer_addition_depth_layer_trend <-
  inner_join(buffer_addition_depth_layer,
             biomemask) %>%
  filter(variable %in% buffer_variables_addition[buffer_variables_addition != "temp"]) %>%
  group_by(depth, biome, tref, variable) %>%
...
for (i_var in buffer_variables_addition[buffer_variables_addition != "temp"]) {
  ...
  p_zonal <- buffer_addition_depth_layer_zonal %>%
    filter(variable == i_var) %>%
    ggplot(aes(sensitivity, lat, col = tref)) +
    geom_vline(xintercept = i_contour_level, col = "grey50") +
    geom_path() +
  ...
  p_trends <- buffer_addition_depth_layer_trend %>%
    filter(variable == i_var) %>%
    ggplot(aes(pCO2, sensitivity, col = biome, group = biome)) +
    geom_hline(yintercept = i_contour_level, col = "grey50") +
    geom_path() +
    geom_point() +
    scale_x_continuous(
...
biome_co2_speciation <-
  ...
  summarise(across(where(is.numeric), ~ weighted.mean(x = ., w = area))) %>%
  ungroup() %>%
  select(-area) %>%
  mutate(tref = as.factor(tref))
...
biome_co2_speciation <-
  ...
  select(biome, depth, tref, H_free, OmegaAragonite, estimate)
...
for (i_var in OIA_variables[-6]) {
  ...
  #     ungroup()
  #   
  #   acidification_volume <- bind_rows(acidification_volume_global,
  #                                     acidification_volume_regional)
  #   
  #   rm(acidification_volume_global,
  #      acidification_volume_regional)
  #   
...
ocean_volume_total <-
  expand_grid(
    full_join(acidification_trend %>%
                filter(tref == 1994),
              ocean_volume_mask),
    depth_integration = c(500, 1000, 2000, 3000)
  ) %>%
...
acidification_volume_binned_all %>% 
  filter(abs(delta_total_num) >= abs(delta_total_max),
         depth_integration == depth_integration_use) %>% 
...
acidification_volume_binned_all %>%
  ...
  geom_rect(data = . %>% filter(delta_total_num != 0),
            aes(xmax = delta_total_num+0.1,
...
p_vol_H_free <-
  ...
  geom_step(aes(delta_total_num+0.4,
                volume_rel,
                col = tref),
            direction = "vh",
            linewidth = 1)+
  ...
  facet_wrap(~ variable,
             scales = "free_x",
             labeller = labeller(variable = x_axis_labels)) +
  labs(y = "Acidified volume (% of top 500m)") +
  theme(
    strip.background = element_blank(),
    strip.text.x = element_markdown(),
...
p_vol_tco2 <-
  ...
  # geom_rect(aes(xmax = delta_total_num+10,
  #               xmin = delta_total_num,
  #               ymax = volume_rel,
  #               ymin = volume_rel,
  #               col = tref),
  #           linewidth = 1) +
  geom_step(aes(delta_total_num+10,
                volume_rel,
                col = tref),
            direction = "vh",
            linewidth = 1) +
...
# # ggsave(
# #   here::here(
# #     "output/all_variables/acidification_volumes_time_series.png"
# #   ),
# #   width = 10,
# #   height = 8,
# #   dpi = 600,
...
# # ggsave(
# #   here::here(
# #     "output/all_variables/acidification_volume_changes.png"
# #   ),
# #   width = 8,
# #   height = 6,
# #   dpi = 600,
...
# # ggsave(
# #   here::here(
# #     "output/all_variables/acidification_volumes_time_series_biome.png"
# #   ),
# #   width = 10,
# #   height = 8,
# #   dpi = 600,
...
# # ggsave(
# #   here::here(
# #     "output/all_variables/acidification_volume_changes_biomes.png"
# #   ),
# #   width = 8,
# #   height = 6,
# #   dpi = 600,
...
# ggsave(
#   here::here(
#     "output/main/acidification_volume_distribution_absolute.png"
#   ),
#   width = 8,
#   height = 6,
#   dpi = 600,
...
# ggsave(
#   here::here(
#     "output/main/acidification_volume_distribution_delta_total.png"
#   ),
#   width = 8,
#   height = 6,
#   dpi = 600,
...
# ggsave(
#   here::here(
#     "output/main/acidification_volume_depth_distribution_delta_total.png"
#   ),
#   width = 8,
#   height = 6,
#   dpi = 600,
...
ggsave(
  here::here(
    paste0(
      "output/main/saturation_horizon_change_aragonite_all_sat_level_2014_global_section.png"
    )
  ),
  width = 8,
  height = 7,
...
ggsave(
  here::here("output/supplement/volume_omega_saturation_basin.png"),
  width = 10,
  height = 4.5,
  dpi = 600,
  bg = "white"
...
acidification_volume_print %>%
  filter(depth_integration == depth_integration_use) %>%
  mutate(
    estimate = case_when(
      estimate == "Saturated volume\n(% total vol.)" ~
        "Saturated volume\n(% top 500m vol.)",
...
ggsave(
  here::here("output/main/volume_omega_saturation_global_OSM.png"),
  width = 5,
  height = 5,
  dpi = 600,
  bg = "white"
...
acidification_global_section_total <-
  ...
  filter(name %in% OIA_variables[-6])
...
absolute_global_section_total <-
  acidification_global_section %>%
  select(tref:band, dist,
         contains(paste0(OIA_variables[-6],"_mean"))) %>%
  filter(tref != 1800)
...
```{r profiles_DIC_dCant_temp_change_biome}
...
detrended_annual %>%
  ...
  scale_y_continuous(trans = trans_reverser("sqrt")) +
  coord_cartesian(expand = 0) +
  facet_grid(DIC_variant ~ station)
...
bind_rows(
  TS_trends %>%
    filter(term == "year") %>%
    select(station, DIC_variant, variable = name, depth = depth_int, estimate, std.error) %>%
    mutate(
      driver = "observed",
      tref = "trend over\nobserved\nperiod",
...
```{r profiles_total_change_biome_interval}
...
ggsave(
  here::here(
    paste0(
      "output/main/profiles_total_changes_global_OSM.png"
    )
  ),
  width = 4,
  height = 6,
